...
class TestSendNotifications:
    ...
    @patch("app.cli.notify_trip_started")
    @patch("app.cli.notify_trip_starting_soon")
    def test_multiple_matching_trips(self, mock_soon, mock_started, mock_ended,
                                     app, make_trip):
        make_trip(destination="SoonTrip", start_date=date(2026, 3, 8),
                  end_date=date(2026, 3, 12))
        make_trip(destination="TodayTrip", start_date=date(2026, 3, 5),
