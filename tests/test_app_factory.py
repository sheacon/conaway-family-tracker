...
class TestGroupFlightsFilter:
    def test_same_flights_grouped(self, app, make_person, make_trip, make_flight):
        p1 = make_person(name="Alice")
        p2 = make_person(name="Bob")
        t = make_trip(destination="London", people=[p1, p2])
        make_flight(t, p1, outbound="WN209", ret="WN4630")
        make_flight(t, p2, outbound="WN209", ret="WN4630")
        filt = app.jinja_env.filters["group_flights"]
        result = filt(t.flight_info)
        assert len(result) == 1
        assert sorted(result[0]["names"]) == ["Alice", "Bob"]
        assert result[0]["outbound"] == "WN209"
        assert result[0]["return"] == "WN4630"

    def test_different_flights_separate(self, app, make_person, make_trip, make_flight):
        p1 = make_person(name="Alice")
        p2 = make_person(name="Bob")
        t = make_trip(destination="London", people=[p1, p2])
        make_flight(t, p1, outbound="UA100", ret="UA200")
        make_flight(t, p2, outbound="BA300", ret="BA400")
        filt = app.jinja_env.filters["group_flights"]
        result = filt(t.flight_info)
        assert len(result) == 2
    ...
    def test_partial_match_not_grouped(self, app, make_person, make_trip, make_flight):
        p1 = make_person(name="Alice")
        p2 = make_person(name="Bob")
        t = make_trip(destination="London", people=[p1, p2])
        make_flight(t, p1, outbound="WN209", ret="WN4630")
        make_flight(t, p2, outbound="WN209", ret="WN999")
        filt = app.jinja_env.filters["group_flights"]
        result = filt(t.flight_info)
        assert len(result) == 2
